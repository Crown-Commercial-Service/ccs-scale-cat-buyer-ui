...
document.addEventListener('DOMContentLoaded', () => {
  if (
    document.getElementById('ccs_rfp_exit_strategy_form') !== null ||
    document.getElementById('ccs_eoi_vetting_form') !== null
  ) {
    if (
...
const ccsZvalidateRfPStrategy = (event) => {
      ...
      if ($('#rfp_prob_statement_m').val().length === 0) {
        if (agreement_id == 'RM1043.8' && group_id == 'Group 13') {
          ...
          if (
            ...
            group_id == 'Group 8' &&
            pageHeading.trim().toLowerCase() == 'The business problem you need to solve'.toLowerCase()
          ) {
    ...
    if (!(pageHeading.includes('(Optional)') || pageHeading.includes('(optional)'))) {
      if ($('#rfp_prob_statement_n').val().length === 0) {
        if (
          ...
          group_id == 'Group 10' &&
          pageHeading.trim().toLowerCase() == 'How often research will happen'.toLowerCase()
        ) {
          fieldCheck = ccsZvalidateTextArea(
            'rfp_prob_statement_n',
            'Enter details about how often research will happen',
          );
    ...
    if (!pageHeading.includes('(Optional)')) {
      if ($('#rfp_prob_statement_d').val().length === 0) {
        if (pageHeading.trim().toLowerCase() == 'Add background to your project'.toLowerCase()) {
          fieldCheck = ccsZvalidateTextArea(
            'rfp_prob_statement_d',
            'Add background information about your procurement',
          );
        } else {
          fieldCheck = ccsZvalidateTextArea(
            'rfp_prob_statement_d',
            'You must add background information about your procurement',
          );
        }
        if (fieldCheck !== true) errorStore.push(fieldCheck);
      }
  ...
  if (
    $('#rfp_prob_statement_s') !== undefined &&
    $('#rfp_prob_statement_s').val() !== undefined &&
    !(pageHeading.includes('(optional)') || pageHeading.includes('(Optional)')) &&
    agreement_id !== 'RM6187'
  ) {
    if ($('#rfp_prob_statement_s').val().length === 0 && pageHeading.includes('Number of research rounds')) {
      ...
      $('#rfp_prob_statement_s').val().length === 0 &&
      pageHeading.includes('Number of participants per round')
    ) {
      ...
      $('#rfp_prob_statement_s').val().length === 0 &&
      pageHeading.includes('Description of your participants')
    ) {
      if (agreement_id == 'RM1043.8') {
      ...
      $('#rfp_prob_statement_s').val().length === 0 &&
      pageHeading.includes('Add background to your project')
    ) {
      fieldCheck = ccsZvalidateTextArea(
        'rfp_prob_statement_s',
        'Add the social value, economic and environmental benefits of your procurement',
      );
      if (fieldCheck !== true) errorStore.push(fieldCheck);
    } else if (
      $('#rfp_prob_statement_s').val().length === 0 &&
      pageHeading.includes('Management information and reporting')
    ) {
  ...
  if (
    ...
    if ($('#rfp_prob_statement_s').val().length === 0) {
      fieldCheck = ccsZvalidateTextArea(
        'rfp_prob_statement_s',
        'Enter management information and reporting requirements',
      );
      if (fieldCheck !== true) errorStore.push(fieldCheck);
    }
  }
  ...
  if (
    ...
    if ($('#rfp_prob_statement_s').val().length === 0) {
      fieldCheck = ccsZvalidateTextArea(
        'rfp_prob_statement_s',
        'You must enter the social value, economic and environmental benefits',
      );
      if (fieldCheck !== true) errorStore.push(fieldCheck);
    }
  }
  ...
  if (
    $('#rfp_security_confirmation') !== undefined &&
    $('#rfp_security_confirmation').val() !== undefined &&
    $("input[name='ccs_vetting_type']").prop('checked')
  ) {
    // errorStore.length = 0;
  ...
  if ($('#rfp_prob_statement_e') !== undefined && $('#rfp_prob_statement_e').val() !== undefined) {
    ...
    if (!pageHeading.includes('(optional)') && !pageHeading.includes('(Optional)') && agreement_id === 'RM6187') {
      if (!pageHeading.includes('Management information and reporting') && !pageHeading.includes('The business need')) {
        if ($('#rfp_prob_statement_e').val().length === 0) {
          fieldCheck = ccsZvalidateTextArea(
            'rfp_prob_statement_e',
            'You must add background information about your procurement',
          );
          if (fieldCheck !== true) errorStore.push(fieldCheck);
        }
      } else if (
        !pageHeading.includes('The business need') &&
        !pageHeading.includes('Add background to your procurement')
      ) {
        if ($('#rfp_prob_statement_e').val().length === 0) {
          fieldCheck = ccsZvalidateTextArea(
            'rfp_prob_statement_e',
