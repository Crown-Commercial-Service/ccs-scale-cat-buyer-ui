  ...
  static AFTER_UPDATINGDATA = async (
    ...
    try {
        ...
        if (isMandatory) {
          ...
          for (let k = 0; k < question_api_data.length; k++) {//multiple questions on page
            ...
            if (questionType == 'Text' || questionType == 'Percentage') {
              ...
              if (dateValidation >= 1)//3 for day,month,year
              {
                innerMandatoryNum += 1;
              }
            }
      ...
      if (check_for_overflowing) {
        ...
        let base_url = `/da/questions?agreement_id=${agreement_id}&proc_id=${proc_id}&event_id=${event_id}&id=${next_criterian_id}&group_id=${next_group_id}&section=''`;
  ...
  static AFTER_UPDATINGDATA_DA_Assessment = async (
    ...
    try {
      ...
      if (check_for_overflowing && current_cursor != -1) {
        ...
        let base_url = `/da/assesstment-question?agreement_id=${agreement_id}&proc_id=${proc_id}&event_id=${event_id}&id=${next_criterian_id}&group_id=${next_group_id}&section=${res.req?.query?.section}=&step${res.req?.query?.step}`;
        res.redirect(base_url);
      } else {
        res.redirect('/da/task-list');
        //}
