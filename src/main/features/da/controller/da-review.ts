...
const DA_REVIEW_RENDER_TEST = async (
  req: express.Request,
  ...
  try {
    ...
    if (agreementId_session == 'RM6263') {
      ...
      technicalgroupquestion1 = sectionbaseURLfetch_dynamic_api_data
      ...
      culturalgroupquestion1 = sectionbaseURLfetch_dynamic_api_data
        ...
        .map((o) => o.nonOCDS)[0].options[0]?.value;
      socialvaluegroupquestion1 = sectionbaseURLfetch_dynamic_api_data
    ...
    if (agreementId_session == 'RM6263') {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o?.OCDS?.id == 'Question 4')?.[0]
        .nonOCDS?.options?.forEach((element) => {
          socialGroup[j].weight = element.value;
          j = j + 1;
        });
      j = 0;
      sectionbaseURLfetch_dynamic_api_data
    ...
    const tierData = tierInfo?.filter((o) => o?.OCDS?.id == 'Question 1')?.[0]?.nonOCDS?.options?.[0].tableDefinition
      ?.titles?.rows;
    // let tierData=[];
    ...
    const startdate = dateOptions != null && dateOptions?.length > 0 ? dateOptions?.[0].value?.padStart(2, 0) : null;
    ...
    if (agreement_id == 'RM6263') {
      ...
      if (sectionbaseURLfetch_dynamic_api_data.length > 0) {
        for (let i = 0; i < sectionbaseURLfetch_dynamic_api_data.length; i++) {
          const curObj = sectionbaseURLfetch_dynamic_api_data[i];
    ...
    if (agreement_id == 'RM6263') {
      ...
      sectionbaseURLfetch_dynamic_api_data?.[1]?.nonOCDS?.options?.forEach((element) => {
        reqGroup?.[i].title = element.value;
        i = i + 1;
      });
      i = 0;
    ...
    let appendData = {
      ...
      rfp_clarification_date:
        rfp_clarification_date != undefined && rfp_clarification_date != null
          ? moment(rfp_clarification_date, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY')
          : null,
      rfp_clarification_period_end:
        rfp_clarification_period_end != undefined && rfp_clarification_period_end != null
          ? moment(rfp_clarification_period_end, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      deadline_period_for_clarification_period:
        deadline_period_for_clarification_period != undefined && deadline_period_for_clarification_period != null
          ? moment(deadline_period_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_period_for_clarification_period:
        supplier_period_for_clarification_period != undefined && supplier_period_for_clarification_period != null
          ? moment(supplier_period_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_for_clarification_period:
        supplier_dealine_for_clarification_period != undefined && supplier_dealine_for_clarification_period != null
          ? moment(supplier_dealine_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_evaluation_to_start:
        supplier_dealine_evaluation_to_start != undefined && supplier_dealine_evaluation_to_start != null
          ? moment(supplier_dealine_evaluation_to_start, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_expect_the_bidders:
        supplier_dealine_expect_the_bidders != undefined && supplier_dealine_expect_the_bidders != null
          ? moment(supplier_dealine_expect_the_bidders, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_for_pre_award:
        supplier_dealine_for_pre_award != undefined && supplier_dealine_for_pre_award != null
          ? moment(supplier_dealine_for_pre_award, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_for_expect_to_award:
        supplier_dealine_for_expect_to_award != undefined && supplier_dealine_for_expect_to_award != null
          ? moment(supplier_dealine_for_expect_to_award, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_sign_contract:
        supplier_dealine_sign_contract != undefined && supplier_dealine_sign_contract != null
          ? moment(supplier_dealine_sign_contract, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
      supplier_dealine_for_work_to_commence:
        supplier_dealine_for_work_to_commence != undefined && supplier_dealine_for_work_to_commence != null
          ? moment(supplier_dealine_for_work_to_commence, 'YYYY-MM-DD HH:mm').format('DD MMMM YYYY, HH:mm')
          : null,
    ...
    if (req.session.selectedSuppliersDA != undefined) {
      ...
      const supplierInfo = supplierListGet.filter(
        (el: any) => el.organization.id == req.session.selectedSuppliersDA
      )?.[0];
      ...
      const actualStatus = journeySteps.find((d) => d.step == 35)?.state;
      ...
      if (
        ...
        ReviewData.OCDS.status != 'published' &&
...
export const POST_DA_REVIEW = async (req: express.Request, res: express.Response) => {
  ...
  if (req.session.selectedSuppliersDA != undefined) {
    ...
    const supplierDataToSave = [];
    const supplierInfo = supplierList.filter((el: any) => el.organization.id == req.session.selectedSuppliersDA)?.[0];
    if (supplierInfo != undefined) {
      supplierDataToSave.push({ name: supplierInfo.organization.name, id: req.session.selectedSuppliersDA });
    }
    ...
    const response = await TenderApi.Instance(req.cookies.SESSION_ID).put(
      `journeys/${req.session.eventId}/steps/34`,
      'Completed'
    );
  ...
  if (review_publish == 1) {
    try {
      ...
      if (agreementId_session == 'RM1557.13') {
        const response = TenderApi.Instance(SESSION_ID).put(BASEURL, _bodyData);
        setTimeout(function () {
          res.redirect('/da/da-eventpublished');
        }, 5000);
      } else {
        const response = await TenderApi.Instance(SESSION_ID).put(BASEURL, _bodyData);
        //CAS-INFO-LOG
      ...
      LoggTracer.errorLogger(
        ...
        null,
        TokenDecoder.decoder(SESSION_ID),
        'DA Review - Dyanamic framework throws error - Tender Api is causing problem',
        false
      );
