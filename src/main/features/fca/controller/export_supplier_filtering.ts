  ...
  try {
    ...
    for (let i = 0; i < supplierListDwn.length; i++) {
      ...
      contactData['Contact email'] =
        contact?.organization?.contactPoint?.email == undefined ? '-' : contact?.organization?.contactPoint?.email;
      contactData['Contact phone number'] =
        contact?.organization?.contactPoint?.telephone == undefined
          ? '-'
          : contact?.organization?.contactPoint?.telephone;
    ...
    LoggTracer.errorLogger(
      ...
      'Journey service - FCA - Create supplier list page',
      true
    );
  }
};
