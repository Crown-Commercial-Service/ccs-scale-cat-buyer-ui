...
export const CREATE_SUPPLIER_SHORTLIST = async (req: express.Request, res: express.Response) => {
  ...
  try {
    const flag = await ShouldEventStatusBeUpdated(eventId, 75, req);
    if (flag) {
      await TenderApi.Instance(SESSION_ID).put(`journeys/${eventId}/steps/75`, 'Not started');
    }
    ...
    if (nameJourneysts.length > 0) {
      ...
      const flagaddCont = await ShouldEventStatusBeUpdated(eventId, 78, req);
    ...
    journeySteps.forEach(function (element, i) {
      ...
      if (element.step == 76 && element.state == 'Cannot start yet') {
        element.state = 'Optional';
      }
      if (element.step == 77 && element.state == 'Cannot start yet') {
        element.state = 'Optional';
      }
    ...
    const windowAppendData = {
      data: fcaCreateSupplierShortlistContent,
      lotId,
      agreementLotName,
      releatedContent,
      agreementId_session,
    ...
    LoggTracer.errorLogger(
      ...
      'FCA - Journey service - Put failed - task list page',
      true
    );
  }
