...
export default function (app: Application): void {
    ...
    app.get(FCA_PATHS.SELECTED_SERVICES, [AUTH, AgreementDetailsFetchMiddleware.FetchAgreements],fcaController.SELECTED_SERVICES);
    app.post(FCA_PATHS.POST_SELECT_SERVICES, [AUTH, AgreementDetailsFetchMiddleware.FetchAgreements],fcaController.POST_SELECT_SERVICES);
    ...
    app.get(FCA_PATHS.CREATE_SUPPLIER_SHORTLIST, [AUTH, PreMarketEngagementMiddleware.PutPremarket], fcaController.CREATE_SUPPLIER_SHORTLIST); //FCA evet type register --> Middleware -> sample(premarketengagement.ts)
    app.get(FCA_PATHS.SELECT_SERVICES, [AUTH, PreMarketEngagementMiddleware.PutPremarket], fcaController.SELECT_SERVICES);
    app.get(FCA_PATHS.NEXT_STEP, [AUTH, AgreementDetailsFetchMiddleware.FetchAgreements], fcaController.FCA_GET_NEXTSTEPS);
    app.post(FCA_PATHS.NEXT_STEP, [AUTH, AgreementDetailsFetchMiddleware.FetchAgreements], fcaController.FCA_POST_NEXTSTEPS);
    app.get(FCA_PATHS.EXPORT_SUPPLIER_FILTERING, [AUTH], fcaController.EXPORT_SUPPLIER_FILTERING);
    //@GET name your projects

  app.get(FCA_PATHS.GET_NAME_YOUR_PROJECT,[ AUTH, AgreementDetailsFetchMiddleware.FetchAgreements],fcaController.GET_NAME_PROJECT);

  //@GET '/fca/add-collaborators'
  app.get(FCA_PATHS.GET_ADD_COLLABORATOR,[ AUTH, AgreementDetailsFetchMiddleware.FetchAgreements],fcaController.GET_ADD_COLLABORATOR);

  //@GET '/fca/procurement-lead'
  app.get(FCA_PATHS.GET_LEAD_PROCUEMENT,[ AUTH, AgreementDetailsFetchMiddleware.FetchAgreements],fcaController.GET_LEAD_PROCUREMENT);
