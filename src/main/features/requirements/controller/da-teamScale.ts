...
export const DA_GET_TEAM_SCALE = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (assessmentDetail.dimensionRequirements.length > 0) {
      ...
      const objIndex = daTeamScale.form.radioOptions.items.findIndex(obj => obj.value === optionId);
      daTeamScale.form.radioOptions.items[objIndex].selected = true;
    }

    const windowAppendData = { data: daTeamScale, lotId, agreementLotName, choosenViewPath, releatedContent };
