...
export const CA_GET_CHOOSE_SECURITY_REQUIREMENTS = async (req: express.Request, res: express.Response) => {
  ...
  const { releatedContent, isError, eventId, errorText, dimensions, currentEvent, projectId, choosenViewPath } =
    req.session;
  ...
  try {
    ...
    if (dimensionRequirements.length > 0) {
      if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 7).length > 0) {
        if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 7)[0].requirements.length > 0) {
          totalQuantityca = dimensionRequirements
            .filter((x) => x['dimension-id'] === 7)[0]
            .requirements.map((a) => a.weighting)
            .reduce(function (a, b) {
              return a + b;
            });
      ...
      if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2).length > 0) {
        if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2)[0].requirements.length > 0) {
          selectedOption = dimensionRequirements
          ...
          securityQuantity = dimensionRequirements
            .filter((dimension) => dimension['dimension-id'] === 2)[0]
    ...
    const flag = await ShouldEventStatusBeUpdated(eventId, 49, req);
    if (flag) {
...
function checkErrors(selectedNumber, resources, totalQuantityca) {
  ...
  if (!selectedNumber) {
    errorText.push({
      text: 'You must select the highest level of security clearance that staff supplied to the project will need to have.',
    });
  } else if (selectedNumber && ['1', '2', '3', '4'].includes(selectedNumber) && !resources) {
    errorText.push({
      text: 'You must enter the number of staff who will need a lower security and vetting requirement',
    });
  } else if (
    selectedNumber &&
    ['1', '2', '3', '4'].includes(selectedNumber) &&
    (resources < 0 || resources > totalQuantityca - 1)
  ) {
...
export const CA_POST_CHOOSE_SECURITY_REQUIREMENTS = async (req: express.Request, res: express.Response) => {
  ...
  if (isError) {
    ...
    try {
      ...
      const { dimensionRequirements } = assessments;
      if (dimensionRequirements.length > 0) {
        dimension2weighitng = dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2)[0]
          ?.weighting;
        SecQuantityrequirements = dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2)[0]
          ?.requirements;
      } else {
        dimension2weighitng = 10;
      }
      const ca_Quantity = totalQuantityca - resources;
      for (const reqrment of SecQuantityrequirements) {
        reqrmnt = [
          {
