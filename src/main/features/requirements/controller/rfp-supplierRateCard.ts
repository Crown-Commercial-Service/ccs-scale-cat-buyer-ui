...
export const RFP_SUPPLIER_RATECARD = async (req: express.Request, res: express.Response) => {
  ...
  const appendData = { supplierName, assessmentsData,data: fcSupplierRatecardScreenContent, events: req.session.openProjectActiveEvents, historicalEvents: req.session.historicalEvents, releatedContent: releatedContent, contactSupplierDetails: contactSupplierDetails ? contactSupplierDetails: {}}
  res.render('rfp-supplier-rate-card', appendData);
} catch (error) {
  LoggTracer.errorLogger(res, error, `${req.headers.host}${req.originalUrl}`, null, TokenDecoder.decoder(SESSION_ID), 'FC Shortlist services - FC task list page', true);
}
}