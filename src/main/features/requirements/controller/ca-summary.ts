    ...
    if (dimensionRequirements.length > 0) {
      ...
      if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2).length > 0) {
        if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 2)[0].requirements.length > 0) {
          Securityoption = dimensionRequirements
            .filter((dimension) => dimension['dimension-id'] === 2)[0]
            .requirements[0].values?.find((y) => y['criterion-id'] == 0)?.value;
          const securityQuantity = dimensionRequirements
            .filter((dimension) => dimension['dimension-id'] === 2)[0]
      ...
      if (dimensionRequirements.filter((x) => x['dimension-id'] == 3)[0]?.requirements.length > 0) {
        const SerCapdimensions = dimensionRequirements.filter((x) => x['dimension-id'] == 3)[0]?.requirements;

        const SerCapRequirements = [];
        SerCapdimensions.forEach((item) => {
          SerCapRequirements.push(CAPACITY_CONCAT_OPTIONS.find((x) => x['requirement-id'] == item['requirement-id']));
        });
        SerCapdimensions.forEach((item) => {
          const index = SerCapRequirements.findIndex((x) => x['requirement-id'] == item['requirement-id']);
        ...
        const distinctgroupnames = [...new Set(individualGroupcluster.map((x) => x.groups[0].name))];
        distinctgroupnames.forEach((item) => {
          const groupname = individualGroupcluster.map((x) => {
            return [item, individualGroupcluster.filter((x) => x.groups[0].name == item)];
          })[0];
      ...
      const eventResponse = await TenderApi.Instance(SESSION_ID).get(
