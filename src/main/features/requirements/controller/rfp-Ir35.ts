...
export const RFP_GET_I35: express.Handler = async (req: express.Request, res: express.Response) => {
  ...
  try {
   ...
   let text=ResponseData[0].nonOCDS;
    ...
    if(flag)
    {
    await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/31`, 'In progress');
    }
...
export const RFP_POST_I35: express.Handler = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/31`, 'Completed');
    let flag=await ShouldEventStatusBeUpdated(req.session.eventId,32,req);
    if(flag){
    await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/32`, 'Not started');
    }
