...
import * as cmsData from '../../../resources/content/requirements/caUploadDocs.json';
...
export const CA_GET_UPLOAD_SUPPORTING_DOCUMENT = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/56`, 'In progress');
