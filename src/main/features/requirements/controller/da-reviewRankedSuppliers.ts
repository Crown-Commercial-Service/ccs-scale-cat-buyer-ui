  ...
  try {
    ...
    if (SuppliersData.suppliers.length > 0) {
      ...
      SuppliersData.suppliers.forEach((element) => {
        const supplierindex = RankedSuppliers.findIndex((x) => x.supplier.id == element.id);
        if (supplierindex > 0) {
    ...
    //DOWNLOAD PRICING INFORMATION
      ...
      const { data: SuppliersPricingData } = await TenderApi.Instance(SESSION_ID).get(pricing_BASEURL);
      res.header('Content-Type', 'text/csv');
      res.attachment('Suppliers_List.csv');
      res.send(SuppliersPricingData);
    } else {
      //PAGINATION
      const noOfPages = Math.ceil(RankedSuppliers.length / rowCount);
      if (previous == undefined && next == undefined) {
        ...
        if (previous != undefined) {
          const currentpagenumber = req.session.DASupplierpagenumber;
...
export const DA_POST_REVIEW_RANKED_SUPPLIERS = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const DARankedSuppliers = req.session.DARankedSuppliers;
    let overallJustification = '';

    if (supplierID != undefined && justification != undefined) {
      overallsuppliers = DARankedSuppliers.filter((y) => y.supplier.id == supplierID).map((x) => {
    ...
    if (response.status == 200) {
      ...
      const flag = await ShouldEventStatusBeUpdated(eventId, 72, req);
      if (flag) {
