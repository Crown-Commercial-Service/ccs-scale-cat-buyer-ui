  ...
  try {
    ...
    if (ca_next_steps) {
      ...
      switch (ca_next_steps) {
        case 'yes':
          console.log('inside yes case');
          await TenderApi.Instance(SESSION_ID).put(`journeys/${eventId}/steps/58`, 'Completed');
          console.log('after tender api');
          console.log('redirect url --- ', REQUIREMENT_PATHS.CA_REQUIREMENT_TASK_LIST);
        ...
        case 'no':
          await TenderApi.Instance(SESSION_ID).put(`journeys/${eventId}/steps/58`, 'Completed');
          res.redirect(REQUIREMENT_PATHS.CA_REQUIREMENT_TASK_LIST); // change to 2nd stage cancelled page
          break;
