...
const RFP_REVIEW_RENDER_STAGE = async (
  req: express.Request,
  ...
  try {
    if (agreementId_session == 'RM1043.8') {
      ...
      const flag = await ShouldEventStatusBeUpdated(event_id, 41, req);
      if (flag) {
    ...
    //console.log("test")
    const fileNameStorageAdditonalDoc = [];
    const fileNameStoragePrice = [];
    const fileNameStorageMandatory = [];
    const fileNameStorageMandatorySecond = [];
    ...
    const rfp_clarification_date = fetchQuestionsData
      ?.filter((item) => item?.OCDS?.id == 'Question 1')
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;
    const rfp_clarification_period_end = fetchQuestionsData
      ?.filter((item) => item?.OCDS?.id == 'Question 2')
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;

    const deadline_period_for_clarification_period = fetchQuestionsData
      ?.filter((item) => item?.OCDS?.id == 'Question 3')
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;

    const supplier_period_for_clarification_period = fetchQuestionsData
    ...
    const supplier_dealine_for_clarification_period = fetchQuestionsData
      ?.filter((item) => item?.OCDS?.id == 'Question 5')
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;

    const supplier_dealine_evaluation_to_start = fetchQuestionsData
    ...
    const supplier_dealine_for_expect_to_award = fetchQuestionsData
      ...
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;
    const supplier_dealine_sign_contract = fetchQuestionsData
      ?.filter((item) => item?.OCDS?.id == 'Question 10')
      .map((item) => item?.nonOCDS?.options)?.[0]
      ?.find((i) => i?.value)?.value;
    const supplier_dealine_for_work_to_commence = fetchQuestionsData
    ...
    if (agreement_id == 'RM1043.8') {
      supplier_start = fetchQuestionsData
        ?.filter((item) => item?.OCDS?.id == 'Question 12')
        .map((item) => item?.nonOCDS?.options)?.[0]
        ?.find((i) => i?.value)?.value;
      supplier_sign_contract = fetchQuestionsData
    ...
    let highestSecurityCount = 0,
      highestSecuritySelected = '';
    ...
    if (agreementId_session != 'RM1043.8') {
      ...
      if (dimensionRequirements?.length > 0) {
        resourceQuantity = dimensionRequirements?.filter((dimension) => dimension.name === 'Resource Quantities')?.[0]
          ?.requirements;
        highestSecurityCount = dimensionRequirements?.filter(
          (dimension) => dimension.name === 'Security Clearance'
        )?.[0]?.requirements?.[0]?.weighting;
        highestSecuritySelected = dimensionRequirements?.filter(
          (dimension) => dimension.name === 'Security Clearance'
        )?.[0]?.requirements?.[0]?.values?.[0]?.value;
        if (highestSecuritySelected === '0: None') highestSecuritySelected = '0: No security clearance needed';
        serviceCapabilitesCount = dimensionRequirements?.filter(
          (dimension) => dimension.name === 'Service Offering'
        )?.[0]?.requirements?.length;
        whereWorkDone = dimensionRequirements
          ?.filter((dimension) => dimension.name === 'Location')?.[0]
          ?.requirements?.map((n) => n.name);
      }
    ...
    //section 5
    //question 2
    const sectionbaseURL: any = '';
    const sectionbaseURLfetch_dynamic_api = [];
    const sectionbaseURLfetch_dynamic_api_data = [];

    const overallratioQuestion1 = '';
    const overallratioQuestion2 = '';

    const culturalgroupquestion1 = '';
    const pricegroupquestion1 = '';
    const technicalgroupquestion1 = '';
    const socialvaluegroupquestion1 = '';
    ...
    const backgroundArr = [];
    const businessProbAns = [];
    const businessProbAnsdata = [];

    //question 5
    const keyUser = [];
    //current phase of project
    const workcompletedsofar = '';
    const currentphaseofproject = '';

    const phaseResource = [];
    const researchDate = [];
    const oftenresearchDate = [];
    const weekendresearchDate = [];
    const weekendresearchDateValue = [];
    const researchLoc = [];
    const restrictLoc = [];
    const descPart = [];
    const digiaccess = [];
    ...
    let furtherInfo;
    const startdate = [];
    const indicativedurationYear = '';
    const indicativedurationMonth = '';
    const indicativedurationDay = '';
    const extentionindicativedurationYear = '';
    const extentionindicativedurationMonth = '';
    const extentionindicativedurationDay = '';
    const buyingorg1 = '';
    const buyingorg2 = '';
    const summarize = '';
    ...
    let appendData = {
      ...
      procurementColleagues:
        procurementColleagues != undefined && procurementColleagues != null ? procurementColleagues : null,
      ...
      supplier_sign_contract:
        supplier_sign_contract != undefined && supplier_sign_contract != null ? supplier_sign_contract : null,
      ...
      StorageForSortedItems:
        StorageForSortedItems != undefined && StorageForSortedItems != null ? StorageForSortedItems : null,
      StorageForServiceCapability:
        StorageForServiceCapability != undefined && StorageForServiceCapability != null
          ? StorageForServiceCapability
          : null,
      checkboxerror: checkboxerror != undefined && checkboxerror != null ? checkboxerror : null,
      highestSecurityCount:
        highestSecurityCount != undefined && highestSecurityCount != null ? highestSecurityCount : null,
      highestSecuritySelected:
        highestSecuritySelected != undefined && highestSecuritySelected != null ? highestSecuritySelected : null,
      serviceCapabilitesCount:
        serviceCapabilitesCount != undefined && serviceCapabilitesCount != null ? serviceCapabilitesCount : null,
      whereWorkDone: whereWorkDone != undefined && whereWorkDone != null ? whereWorkDone : null,
      overallratioQuestion1:
        overallratioQuestion1 != undefined && overallratioQuestion1 != null ? overallratioQuestion1 : null,
      overallratioQuestion2:
        overallratioQuestion2 != undefined && overallratioQuestion2 != null ? overallratioQuestion2 : null,
      technicalgroupquestion1:
        technicalgroupquestion1 != undefined && technicalgroupquestion1 != null ? technicalgroupquestion1 : null,
      culturalgroupquestion1:
        culturalgroupquestion1 != undefined && culturalgroupquestion1 != null ? culturalgroupquestion1 : null,
      ...
      socialvaluegroupquestion1:
        socialvaluegroupquestion1 != undefined && socialvaluegroupquestion1 != null ? socialvaluegroupquestion1 : null,
      ...
      securityRequirements:
        securityRequirements != undefined && securityRequirements != null ? securityRequirements : null,
      ...
      currentphaseofproject:
        currentphaseofproject != undefined && currentphaseofproject != null ? currentphaseofproject : null,
      ...
      startdate:
        startdate != undefined && startdate != null ? moment(startdate, 'YYYY-MM-DD ').format('DD MMMM YYYY') : null,
      ...
      eventStatus:
        ReviewData.OCDS.status == 'active' ? 'published' : ReviewData.OCDS.status == 'complete' ? 'published' : null, // this needs to be revisited to check the mapping of the planned
...
const RFP_REVIEW_RENDER_TEST = async (
  ...
  try {
    ...
    const IR35selected = IR35Data?.[0].nonOCDS?.options
    ...
    const overallratioQuestion2 = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.[0]?.value;
    ...
    const technicalgroupquestion1 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS?.order == 1)
      .map((o) => o.nonOCDS)[0].options[0]?.value;
    if (agreementId_session == 'RM6263') {
      // DSP
      culturalgroupquestion1 = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS?.order == 2)
        .map((o) => o.nonOCDS)[0].options[0]?.value;
    }
    const socialvaluegroupquestion1 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS?.order == 3)
      .map((o) => o.nonOCDS)[0].options[0]?.value;
    if (agreementId_session == 'RM1043.8') {
      //DOS
      culturalgroupquestion1 = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS?.order == 2)
        .map((o) => o.nonOCDS)[0].options[0]?.value;
      pricegroupquestion1 = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS?.order == 4)
        .map((o) => o.nonOCDS)[0].options[0]?.value;
    }
    ...
    sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.OCDS?.id == 'Question 1')?.[0]
      .nonOCDS?.options?.forEach((element) => {
    ...
    if (agreementId_session == 'RM6263') {
      // DSP
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 2')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          techGroup[j].add = element.value;
          j = j + 1;
        });
    ...
    j = 0;
    sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.OCDS?.id == 'Question 4')?.[0]
      ?.nonOCDS?.options?.forEach((element) => {
        techGroup?.[j].weight = element?.value;
        j = j + 1;
      });
    if (agreementId_session == 'RM6263') {
      // DSP
      j = 0;
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 3')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          techGroup?.[j].good = element?.value;
          j = j + 1;
        });
    } else if (agreementId_session == 'RM1043.8') {
      //DOS
    ...
    if (agreementId_session == 'RM6263') {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS.id == 'Question 4')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          culGroup?.[j].weight = element?.value;
          j = j + 1;
        });
    } else if (agreementId_session == 'RM1043.8') {
      // DSP
    ...
    if (agreementId_session != 'RM1043.8') {
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 4')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          socialGroup[j]?.tech = element.value;
          j = j + 1;
        });
    }
    if (agreementId_session == 'RM6263') {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 2')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          socialGroup[j].add = element.value;
          j = j + 1;
        });
    }
    if (agreementId_session == 'RM1043.8') {
    ...
    if (agreementId_session == 'RM6263') {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o?.OCDS?.id == 'Question 3')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          socialGroup?.[j].good = element?.value;
          j = j + 1;
        });
    }
    const pricingModel = [];
    if (agreementId_session == 'RM1043.8') {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 2')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          pricingModel[j].add = element.value;
          j = j + 1;
        });
    } else {
    ...
    if (agreementId_session != 'RM1043.8') {
      ...
      const tierInfo = await CalScoringCriteria(req);
      const newfilteredData = tierInfo?.filter((o) => o?.OCDS?.id == 'Question 1')?.[0]?.nonOCDS?.options?.[0]
        ?.tableDefinition;
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 4')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          socialModel[j].add = element.value;
          j = j + 1;
        });
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
      ...
      sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
      priModel = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter((o) => o.selected == true)?.[0]
        ?.value;
    }
    let assessModel = [];
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
      ...
      questionModel = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter((o) => o.selected == true)?.[0]
        ?.value;
      sectionbaseURL = `/tenders/projects/${proc_id}/events/${event_id}/criteria/Criterion 2/groups/Group 13/questions`;
      sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api_data.sort(
        (n1, n2) => n1.nonOCDS.order - n2.nonOCDS.order
      );
      ...
      j = 0;
      TemporaryObjStorage?.filter(
        (o) => o?.OCDS?.id == 'Question 1'
      )?.[0]?.nonOCDS?.options?.[0].optiontableDefination?.rows?.forEach((element) => {
    ...
    sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api_data.sort((a, b) =>
      a.OCDS.id < b.OCDS.id ? -1 : 1
    );
    ...
    if (data != undefined) {
      data?.forEach((element) => {
        const info = { text: element.text, value: element.value };
    ...
    if (agreementId_session == 'RM1043.8' && (req.session.lotId == 3 || req.session.lotId == 1)) {
      ...
      sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
      backgroundArr = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter((o) => o.selected == true)?.[0]
        ?.value;
    }
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      businessProbAns = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else {
      businessProbAns = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o?.nonOCDS?.order == 1)
        .map((o) => o.nonOCDS)?.[0].options?.[0]?.value;
    }
    ...
    if (agreementId_session == 'RM6263') {
      ...
      keyUser = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o?.nonOCDS?.order == 1).map((o) => o.nonOCDS)?.[0]
        .options?.[0]?.value;
    }
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
      sectionbaseURL = `/tenders/projects/${proc_id}/events/${event_id}/criteria/Criterion 3/groups/Group 6/questions`;
    ...
    const workcompletedsofar = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS?.order == 1)
      .map((o) => o.nonOCDS)?.[0].options?.[0]?.value;
    ...
    if (agreementId_session != 'RM6187') {
      ...
      currentphaseofproject = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter(
        (o) => o.selected == true
      )?.[0]?.value;
    }
    ...
    if (agreementId_session == 'RM6263') {
      ...
      phaseResource = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    }
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.OCDS?.id == 'Question 2')?.[0]
        ?.nonOCDS?.options?.forEach((element) => {
          phaseResource[j].add = element.value;
          j = j + 1;
        });
    }
    let researchDate = [];
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      researchDate = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      oftenresearchDate = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
      j = 0;
      weekendresearchDate = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      researchLoc = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      restrictLoc = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      descPart = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    } else if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 3) {
      ...
      const data = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 1)?.[0]?.nonOCDS?.options;
      if (data != undefined) {
        data?.forEach((element) => {
          const info = { value: element.value };
      ...
      researchPlan = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
      ...
      const termdata = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 2)?.[0]?.nonOCDS?.options;
      ...
      budget = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
      budgetMaximum = sectionbaseURLfetch_dynamic_api_data?.[1].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
      budgetMinimum = sectionbaseURLfetch_dynamic_api_data?.[2].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
      furtherInfo = sectionbaseURLfetch_dynamic_api_data?.[3]?.nonOCDS?.options
        ?.filter((o) => o?.selected == true)
        ?.map((a) => a?.value);
    }
    ...
    if (agreementId_session == 'RM1043.8' && req.session.lotId == 1) {
      ...
      const data = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
      digiaccess = data;
      securityRequirements = sectionbaseURLfetch_dynamic_api_data?.[1]?.nonOCDS?.options
        ?.filter((o) => o?.selected == true)
        ?.map((a) => a?.value);
      sectionbaseURL = `/tenders/projects/${proc_id}/events/${event_id}/criteria/Criterion 3/groups/Group 17/questions`;
      sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
      ...
      const dateOptions = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 1)
        .map((o) => o.nonOCDS)?.[0]?.options;
      ...
      if (dateOptions != null && dateOptions?.length > 0 && dateOptions?.length == 3) {
        startdate =
          dateOptions != null && dateOptions?.length > 0
            ? `${dateOptions?.[2].value}-${dateOptions?.[1].value}-${dateOptions?.[0].value}`?.padStart(2, 0)
            : null;
      } else {
        startdate = dateOptions != null && dateOptions?.length > 0 ? dateOptions?.[0].value?.padStart(2, 0) : null;
      }
      ...
      const optionalDate = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 1)
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;

      const extentionOptionalDate = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 2)
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
      ...
      if (extentionOptionalDate != undefined) {
        ...
        extentionindicativedurationMonth =
          extentionOptionalDate?.substring(1)?.split('Y')?.[1]?.split('M')?.[0] + ' months';
        extentionindicativedurationDay =
          extentionOptionalDate?.substring(1)?.split('Y')?.[1]?.split('M')?.[1].replace('D', '') + ' days';
      }
      ...
      sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
      sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
      contracted = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options
        ?.filter((o) => o.selected == true)
        ?.map((a) => a.value);
    }
    ...
    if (agreementId_session != 'RM1043.8') {
      ...
      const buyingorg2 = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 2)
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
      ...
      const summarize = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 1)
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
      ...
      const newreplace = sectionbaseURLfetch_dynamic_api_data[0].nonOCDS?.options?.filter(
        (o) => o.selected == true
      )?.[0]?.value;
      ...
      const incumbentoption = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.questionType == 'SingleSelect')?.[0]
        ?.nonOCDS?.options?.filter((o) => o.selected == true)?.[0]?.value;
      const suppliername = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.questionType == 'Text')
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
      ...
      const managementinfo = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 1)
        ?.map((o) => o.nonOCDS)?.[0].options?.[0]?.value;
    }
    ...
    const serviceLevel = [];
    const data1 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.OCDS.id == 'Question 1')?.[0]?.nonOCDS?.options;
    const data2 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.OCDS.id == 'Question 2')?.[0]?.nonOCDS?.options;
    const dataPercent = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.OCDS.id == 'Question 3')?.[0]?.nonOCDS
      ?.options;
    ...
    const incentive1 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS.order == 1)
      ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
    const incentive2 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS.order == 1)
      ?.map((o) => o.nonOCDS)?.[0]?.options?.[1]?.value;
    ...
    if (agreementId_session != 'RM1043.8' && req.session.lotId == 3) {
      ...
      bc2 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 2)?.map((o) => o.nonOCDS)?.[0]
        ?.options?.[0]?.value;
      further_info = sectionbaseURLfetch_dynamic_api_data
        ?.filter((o) => o.nonOCDS.order == 3)
        ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
    } else {
      bc1 = undefined;
    ...
    if (agreementId_session != 'RM1043.8') {
      if (overWritePaJoury) {
        ...
        const { data: supplierScoreList } = await TenderApi.Instance(SESSION_ID).get(
          `/assessments/${CurrentassessmentId}?scores=true`
        );
        const dataSet = supplierScoreList.dimensionRequirements;
        if (dataSet.length > 0) {
          const dataRequirements = dataSet[0].requirements;
    ...
    if (agreementId_session == 'RM1043.8') {
      ...
      appendData = {
        ...
        rfp_clarification_date:
          rfp_clarification_date != undefined && rfp_clarification_date != null
            ? moment(rfp_clarification_date, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY')
            : null,
        ...
        rfp_clarification_period_end:
          rfp_clarification_period_end != undefined && rfp_clarification_period_end != null
            ? moment(rfp_clarification_period_end, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        deadline_period_for_clarification_period:
          deadline_period_for_clarification_period != undefined && deadline_period_for_clarification_period != null
            ? moment(deadline_period_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_period_for_clarification_period:
          supplier_period_for_clarification_period != undefined && supplier_period_for_clarification_period != null
            ? moment(supplier_period_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_for_clarification_period:
          supplier_dealine_for_clarification_period != undefined && supplier_dealine_for_clarification_period != null
            ? moment(supplier_dealine_for_clarification_period, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_evaluation_to_start:
          supplier_dealine_evaluation_to_start != undefined && supplier_dealine_evaluation_to_start != null
            ? moment(supplier_dealine_evaluation_to_start, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_expect_the_bidders:
          supplier_dealine_expect_the_bidders != undefined && supplier_dealine_expect_the_bidders != null
            ? moment(supplier_dealine_expect_the_bidders, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_for_pre_award:
          supplier_dealine_for_pre_award != undefined && supplier_dealine_for_pre_award != null
            ? moment(supplier_dealine_for_pre_award, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_for_expect_to_award:
          supplier_dealine_for_expect_to_award != undefined && supplier_dealine_for_expect_to_award != null
            ? moment(supplier_dealine_for_expect_to_award, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_sign_contract:
          supplier_dealine_sign_contract != undefined && supplier_dealine_sign_contract != null
            ? moment(supplier_dealine_sign_contract, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_dealine_for_work_to_commence:
          supplier_dealine_for_work_to_commence != undefined && supplier_dealine_for_work_to_commence != null
            ? moment(supplier_dealine_for_work_to_commence, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_sign_contract:
          supplier_sign_contract != undefined && supplier_sign_contract != null
            ? moment(supplier_sign_contract, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        supplier_start:
          supplier_start != undefined && supplier_start != null
            ? moment(supplier_start, 'YYYY-MM-DD HH:mm').format('DD/MM/YYYY, HH:mm')
            : null,
        resourceQuntityCount:
          resourceQuntityCount != undefined && resourceQuntityCount != null ? resourceQuntityCount : null,
        resourceQuantity: resourceQuantity != undefined && resourceQuantity != null ? resourceQuantity : null,
        ...
        culturalgroupquestion1:
          culturalgroupquestion1 != undefined && culturalgroupquestion1 != null ? culturalgroupquestion1 : null,
        pricegroupquestion1:
          pricegroupquestion1 != undefined && pricegroupquestion1 != null ? pricegroupquestion1 : null,
        socialvaluegroupquestion1:
          socialvaluegroupquestion1 != undefined && socialvaluegroupquestion1 != null
            ? socialvaluegroupquestion1
            : null,
        pricingModel: pricingModel != undefined && pricingModel != null ? pricingModel : null,
        culModel: culModel != undefined && culModel != null ? culModel : null,
        ...
        oftenresearchDate: oftenresearchDate != undefined && oftenresearchDate != null ? oftenresearchDate : null,
        weekendresearchDate:
          weekendresearchDate != undefined && weekendresearchDate != null ? weekendresearchDate : null,
        researchLoc: researchLoc != undefined && researchLoc != null ? researchLoc : null,
        restrictLoc: restrictLoc != undefined && restrictLoc != null ? restrictLoc : null,
        descPart: descPart != undefined && descPart != null ? descPart : null,
        digiaccess: digiaccess != undefined && digiaccess != null ? digiaccess : null,
  ...
  if (review_publish == 1) {
    try {
      ...
      LoggTracer.errorLogger(
        ...
        'Dyanamic framework throws error - Tender Api is causing problem',
        false
      );
...
const RFP_REVIEW_RENDER = async (
  ...
  try {
    ...
    //JSONData;
    const Eoi_answered_questions = BuyerAnsweredAnswers.map((eoi) => eoi.requirement).flat();
...
const RFP_REVIEW_RENDER_TEST_MCF = async (
  req: express.Request,
  res: express.Response,
  viewError: boolean,
  apiError: boolean
  ...
  try {
    ...
    const fileNameStoragePrice = [];
    const fileNameStorageMandatory = [];
    const fileNameStorageAdditional = [];
    FETCH_FILEDATA?.map((file) => {
      if (file.description === 'mandatoryfirst') {
        fileNameStoragePrice.push(file.fileName);
      }
    ...
    if (dimensionRequirements?.length > 0) {
      resourceQuantity = dimensionRequirements?.filter((dimension) => dimension.name === 'Resource Quantities')?.[0]
        ?.requirements;
      highestSecurityCount = dimensionRequirements?.filter((dimension) => dimension.name === 'Security Clearance')?.[0]
        ?.requirements?.[0]?.weighting;
      highestSecuritySelected = dimensionRequirements?.filter(
        (dimension) => dimension.name === 'Security Clearance'
      )?.[0]?.requirements?.[0]?.values?.[0]?.value;
      if (highestSecuritySelected === '0: None') highestSecuritySelected = '0: No security clearance needed';
      serviceCapabilitesCount = dimensionRequirements?.filter((dimension) => dimension.name === 'Service Offering')?.[0]
        ?.requirements?.length;
      whereWorkDone = dimensionRequirements
        ?.filter((dimension) => dimension.name === 'Location')?.[0]
        ?.requirements?.map((n) => n.name);
    }
    const resourceQuntityCount = resourceQuantity?.length;
    StorageForSortedItems = await CalVetting(req);
    let StorageForServiceCapability = [];
    StorageForServiceCapability = await CalServiceCapability(req);
    //section 5
    ...
    const overallratioQuestion1 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS?.order == 1)
      .map((o) => o.nonOCDS)[0].options[0]?.value;
    const overallratioQuestion2 = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS?.order == 2)
      .map((o) => o.nonOCDS)[0].options[0]?.value;
    ...
    const pricingModel = null; //sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter(o => o.selected == true)[0]?.value;
    ...
    const tierData = newfilteredData.titles.rows;
    //  let tierData = tierInfo?.filter(o => o?.OCDS?.id == 'Question 1')?.[0]?.nonOCDS?.options?.[0].tableDefinition?.titles?.rows;
    // let tierData=[];
    // tierRows.forEach(element => {
    //   tierData.push({id:element.id,name:element.name,text:element.text});
    ...
    if (data != undefined) {
      data?.forEach((element) => {
        const info = { text: element.value, value: element.text };
        termAndAcr.push(info);
      });
    }
    ...
    const backgroundArr = [];
    const SortedData = sectionbaseURLfetch_dynamic_api_data.sort(
      (a, b) => parseFloat(b.nonOCDS.order) - parseFloat(a.nonOCDS.order)
    );
    ...
    sectionbaseURLfetch_dynamic_api = await DynamicFrameworkInstance.Instance(SESSION_ID).get(sectionbaseURL);
    sectionbaseURLfetch_dynamic_api_data = sectionbaseURLfetch_dynamic_api?.data;
    const businessProbAns = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o?.nonOCDS?.order == 1)
      .map((o) => o.nonOCDS)?.[0].options?.[0]?.value;
    ...
    if (agreementId_session != 'RM6187') {
      ...
      const currentphaseofproject = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter(
        (o) => o.selected == true
      )?.[0]?.value;
    }
    let phaseResource = [];
    ...
    const newreplace = sectionbaseURLfetch_dynamic_api_data[0].nonOCDS?.options?.filter((o) => o.selected == true)?.[0]
      ?.value;
    ...
    const bc2 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 2)?.map((o) => o.nonOCDS)?.[0]
      ?.options?.[0]?.value;
    const further_info = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS.order == 3)
      ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
...
const RFP_REVIEW_RENDER_GCLOUD = async (
  req: express.Request,
  res: express.Response,
  viewError: boolean,
  apiError: boolean
  ...
  try {
    ...
    const premarket = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS.order == 1)
      ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
    ...
    expanded = sectionbaseURLfetch_dynamic_api_data?.[0].nonOCDS?.options?.filter((o) => o.selected == true)?.[0]
      ?.value;
    ...
    const bc1 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 2)?.map((o) => o.nonOCDS)?.[0]
      ?.options?.[0]?.value;
    const bc2 = sectionbaseURLfetch_dynamic_api_data?.filter((o) => o.nonOCDS.order == 3)?.map((o) => o.nonOCDS)?.[0]
      ?.options?.[0]?.value;
    const further_info = sectionbaseURLfetch_dynamic_api_data
      ?.filter((o) => o.nonOCDS.order == 4)
      ?.map((o) => o.nonOCDS)?.[0]?.options?.[0]?.value;
    ...
    const fetch_dynamic_service_api_data = fetch_dynamic_service_api?.data;
    selectedServices = fetch_dynamic_service_api_data?.[0].nonOCDS?.options
      ?.filter((data) => data.selected == true)
      ?.map((data) => data.value);
