  ...
  try {
    ...
    if (dimensionRequirements.length > 0) {
      if (dimensionRequirements.filter((dimension) => dimension['dimension-id'] === 7).length > 0) {
        totalQuantityrfp = dimensionRequirements
      ...
      if (dimensionRequirements.filter((dimension) => dimension.name === 'Security Clearance').length > 0) {
        ...
        if (
          dimensionRequirements.filter((dimension) => dimension.name === 'Security Clearance')[0].requirements[0]
            .weighting > 0
        ) {
          data.form.selectedValue = dimensionRequirements.filter(
    ...
    const flag = await ShouldEventStatusBeUpdated(req.session.eventId, 34, req);
    if (flag) {
...
function checkErrors(selectedNumber, resources, totalQuantityrfp) {
  ...
  if (!selectedNumber) {
    errorText.push({
      text: 'You must select the highest level of security clearance that staff supplied to the projrct will need to have.',
    });
  } else if (selectedNumber && ['1', '2', '3', '4'].includes(selectedNumber) && !resources) {
    errorText.push({
      text: 'You must enter the number of staffs who will need a lower security and vetting requirement.',
    });
  } else if (
    selectedNumber &&
    ['1', '2', '3', '4'].includes(selectedNumber) &&
    (resources < 0 || resources > totalQuantityrfp - 1)
  ) {
