...
export const RFP_GET_REMOVE_FILES_ATTACHMENT = (express.Handler = async (
  req: express.Request,
...
export const RFP_POST_UPLOAD_ATTACHMENT_PROCEED = (express.Handler = async (
  ...
  if (
    req.session.agreement_id == 'RM1043.8' &&
    req.session?.stage2_value !== undefined &&
    req.session?.stage2_value === 'Stage 2'
  ) {
  ...
  if (req.session['isTcUploaded'] && rfp_confirm_upload === 'confirm') {
    ...
    req.session['pricingSchedule'] = {
      IsDocumentError: true,
      IsFile: !req.session['isTcUploaded'] ? true : false,
      rfp_confirm_upload: rfp_confirm_upload == undefined ? true : false,
    };
    res.redirect('/rfp/upload-attachment');
  }
});
