...
export const CA_GET_REVIEW_RANKED_SUPPLIERS = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (download != undefined) {
      ...
      await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/54`, 'In progress');
...
export const CA_POST_REVIEW_RANKED_SUPPLIERS = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (response.status == 200) {
      await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/54`, 'Completed');
      await TenderApi.Instance(SESSION_ID).put(`journeys/${req.session.eventId}/steps/55`, 'Not started');
