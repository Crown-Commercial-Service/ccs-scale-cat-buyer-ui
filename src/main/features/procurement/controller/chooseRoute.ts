...
export const GET_CHOOSE_ROUTE = async (req: express.Request, res: express.Response) => {
  ...
  const activeProjectID = req.session.projectId;
  const agreement_id = req.session.agreement_id;
  const eventId = req.session.eventId;
  const { data: dataActiveRecord } = await TenderApi.Instance(SESSION_ID).get(
    `/tenders/projects/${activeProjectID}/events`
  );
  ...
  if (agreement_id == 'RM1557.13') {
    ...
    const jsonData = agreement_id == 'RM6187' ? Mcf3cmsData : cmsData;
