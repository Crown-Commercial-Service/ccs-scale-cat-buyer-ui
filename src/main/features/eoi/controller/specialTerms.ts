 ...
 */
 export const GET_EOI_SPECIAL_TERMS = async (req: express.Request, res: express.Response) => {
  ...
  const { lotId, agreementLotName, agreement_id ,releatedContent , errorText , emptyspecialtermError,emptyExpspecialtermError } = req.session;
  let textAreaerrors;
  textAreaerrors  = {
  ...
  const windowAppendData = { data: forceChangeDataJson, error: textAreaerrors, lotId, agreementLotName, releatedContent, agreement_id };
