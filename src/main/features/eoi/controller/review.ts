...
export const POST_EOI_REVIEW = async (req: express.Request, res: express.Response) => {
  ...
  const dateSplited = CurrentTimeStamp.split('*')[1];
  if (momentz(new Date(dateSplited)).tz('Europe/London').isDST()) {
    const dateFormated = momentz(new Date(dateSplited)).format('YYYY-MM-DDTHH:mm:ss+01:00');
    CurrentTimeStamp = momentz(new Date(dateFormated)).toISOString();
  } else {
    const dateFormated = momentz(new Date(dateSplited)).format('YYYY-MM-DDTHH:mm:ss+00:00');
    CurrentTimeStamp = momentz(new Date(dateFormated)).toISOString();
  }
  ...
  if (review_publish == '1') {
    try {
      ...
      LoggTracer.errorLogger(
        ...
        'EOI Review - Dyanamic framework throws error - Tender Api is causing problem',
        false
      );
          ...
          answer: questions.answers.map((answer) => {
            ...
            if (answer.questionType == 'Date') {
              ...
              obj.values = [
                {
                  value:
                    'Date you want the project to start: ' + moment(startDate, 'YYYY-MM-DD ').format('DD MMMM YYYY'),
                  selected: true,
                },
              ];
