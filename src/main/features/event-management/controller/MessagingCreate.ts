...
export const POST_MESSAGING_CREATE = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    let IsClassificationNotDefined,
      IsSubjectNotDefined,
      IsMessageNotDefined,
      validationError = false;
    const errorText = [];
...
export const POST_MESSAGING_SUBBLIER_CREATE = async (req: express.Request, res: express.Response) => {
  ...
  try {
    const _body = req.body;
    let IsSupplierNotDefined,
      IsClassificationNotDefined,
      IsSubjectNotDefined,
      IsMessageNotDefined,
      validationError = false;
    const errorText = [];
    ...
    if (validationError) {
      ...
      for (let i = 0; i < SUPPLIER_DATA.suppliers.length; i++) {
        if (SUPPLIER_DATA.suppliers[i].id == _body.create_supplier_message)
          MatchedSupplierIDS.push(SUPPLIER_DATA.suppliers[i].name);
      }
