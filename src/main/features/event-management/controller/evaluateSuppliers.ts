...
export const EVALUATE_SUPPLIERS = async (req: express.Request, res: express.Response) => {
  ...
  const { agreement_header } = req.session;
...
export const EVALUATE_SUPPLIERS_POPUP = async (req: express.Request, res: express.Response) => {
  ...
  const ScoresAndFeedbackURLdata_: any[] = [];
  ...
  try {
    ...
    const body = ScoresAndFeedbackURLdata_;

    TenderApi.Instance(SESSION_ID).put(
      `/tenders/projects/${projectId}/events/${eventId}/scores?scoring-complete=true`,
      body
    );
...
export const CONFIRM_SCORE_GET = async (req: express.Request, res: express.Response) => {
  ...
  async function statusApis() {
    ...
    const apidata: any = await TenderApi.Instance(SESSION_ID)
      .get(baseurl)
  ...
  do {
    ...
    const status = statusResponse.filter((d: any) => d.id == eventId)[0].dashboardStatus;
