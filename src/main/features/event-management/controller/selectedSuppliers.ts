...
export const INVITE_SELECTED_SUPPLIERS = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    for (let m = 0; m < supplierdata.data.length; m++) {
      if (
        supplierdata.data[m].organisationId == supplierid &&
        supplierdata.data[m].comment != 'No comment found' &&
        supplierdata.data[m].score != null
      ) {
...
export const INVITE_SELECTED_SUPPLIERS_POST = async (req: express.Request, res: express.Response) => {
  ...
  try {
    if (enter_evaluation_feedback && enter_evaluation_score) {
      ...
      await TenderApi.Instance(SESSION_ID).put(`tenders/projects/${projectId}/events/${eventId}/scores`, body);
