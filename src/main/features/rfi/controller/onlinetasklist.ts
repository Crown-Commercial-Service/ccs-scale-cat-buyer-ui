...
export const GET_ONLINE_TASKLIST = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const journeys = journeySteps.find((item) => item.step == 10);
    ...
    let text = '';
    for (let i = 0; i < ExcludingKeyDates.length; i++) {
    ...
    for (let index = 0; index < ExcludingKeyDates.length; index++) {
      ...
      for (let j = 0; j < fetch_dynamic_api_data2.length; j++) {
        ...
        const questionOptions = fetch_dynamic_api_data2[j].nonOCDS.options;
        //let item1 = questionOptions.find(i => i.selected === true);

        const item1 = fetch_dynamic_api_data2[j].nonOCDS.answered;
    ...
    LoggTracer.errorLogger(
      ...
      'RFI Online Task List - Tenders Service Api cannot be connected',
      true
    );
  }
};
