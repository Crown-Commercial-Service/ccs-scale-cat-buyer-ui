...
export const RankSuppliersforDA = async (req: express.Request) => {
  ...
  try {
    ...
    SupplierswithName.forEach((item) => {
      const suppliersinfo = assesssort.filter((x) => x.supplier.id === item.organization.id);
    ...
    if (distinctassessments.length === assesssort.length) {
      ...
      for (let i = 0; i < assesssort.length; i++) {
        const similarRankedItems = assesssort.filter((x) => x.total == assesssort[i].total);
        if (similarRankedItems.length == 1) {
          assesssort[i].rank = ++rankValue;
          assesssort[i].name = supplierList.find(
            (s) => s.organization.id === assesssort[i].supplier.id
          ).organization.name;
          ...
          const firstIterate = similarRankedItems.map((x) => {
            return [x.supplier.id, x.dimensionScores[0]];
          });
          const firstIterateSort: any = firstIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
          ...
          if (firstIterateDistint.length > 1) {
            firstIterateSort.forEach((x) => {
              const indexVal = assesssort.findIndex((sup) => sup.supplier.id == x[0]);
              assesssort[indexVal].name = supplierList.find(
                (s) => s.organization.id === assesssort[indexVal].supplier.id
              ).organization.name;
            ...
            const secondIterate = similarRankedItems.map((x) => {
              return [x.supplier.id, x.dimensionScores[1]];
            });
            const secondIterateSort: any = secondIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
            ...
            if (secondIterateDistint.length > 1) {
              ...
              const thirdIterate = similarRankedItems.map((x) => {
                return [x.supplier.id, x.dimensionScores[2]];
              });
              const thirdIterateSort: any = thirdIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
              ...
              if (thirdIterateDistint.length > 1) {
                ...
                const fourthIterate = similarRankedItems.map((x) => {
                  return [x.supplier.id, x.dimensionScores[3]];
                });
                const fourthIterateSort: any = fourthIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
                ...
                if (fourthIterateDistint.length > 1) {
                  ...
                  const fifthIterate = similarRankedItems.map((x) => {
                    return [x.supplier.id, x.dimensionScores[4]];
                  });
                  const fifthIterateSort: any = fifthIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
                  ...
                  if (fifthIterateDistint.length > 1) {
                    ...
                    const sixthIterate = similarRankedItems.map((x) => {
                      return [x.supplier.id, x.dimensionScores[1]];
                    });
                    const sixthIterateSort: any = sixthIterate.sort((a, b) => (a[1].score > b[1].score ? -1 : 1));
                    ...
                    if (sixthIterateDistint.length > 1) {
                      ...
                      similarRankedItems.forEach((x) => {
                        const indexVal = assesssort.findIndex((sup) => sup.supplier.id == x.supplier.id);
                        assesssort[indexVal].rank = similarrank;
                        assesssort[indexVal].name = supplierList.find(
                          (s) => s.organization.id === assesssort[indexVal].supplier.id
                        ).organization.name;
    ...
    for (let i = 0; i < RankedSuppliers.length; i++) {
      RankedSuppliers[i].dimensionScores.sort((a, b) =>
        a['dimension-id'] < b['dimension-id'] ? -1 : a['dimension-id'] > b['dimension-id'] ? 1 : 0
      );
    }
