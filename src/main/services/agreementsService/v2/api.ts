...
import { EndPoints, AgreementLotEventType, AgreementServiceHealthResponse } from '../../types/agreementsService/v2/api';
  ...
  return genericFecthGet<LotDetail>(
    {
      ...
      params: { agreementId, 'lotId': formattedLotId }
    },
    headers(),
    {
      key: `get_agreements_${agreementId}_lots_${formattedLotId}`,
      seconds: agreementServiceCacheLength
    },
    {
      name: 'agreement service',
      message: `Feached agreement lot from the Agreement service API for agreement: ${agreementId}, lot: ${formattedLotId}`
    }
  );
};
...
const getAgreementLotSuppliers = async (agreementId: string, lotId: string): Promise<FetchResult<LotSupplier[]>> => {
  ...
  return genericFecthGet<LotSupplier[]>(
    ...
    headers(),
    {
      key: `get_agreements_${agreementId}_lots_${formattedLotId}_suppliers`,
      seconds: 900
    },
    {
      name: 'agreement service',
      message: `Feached agreement lot suppliers from the Agreement service API for agreement: ${agreementId}, lot: ${formattedLotId}`
    }
  );
};
...
const getAgreementLotEventTypes = async (agreementId: string, lotId: string): Promise<FetchResult<AgreementLotEventType[]>> => {
  ...
  return genericFecthGet<AgreementLotEventType[]>(
    ...
    headers(),
    {
      key: `get_agreements_${agreementId}_lots_${formattedLotId}_event_types`,
      seconds: agreementServiceCacheLength
    },
    {
      name: 'agreement service',
      message: `Feached agreement lot event types from the Agreement service API for agreement: ${agreementId}, lot: ${formattedLotId}`
    }
  );
};
