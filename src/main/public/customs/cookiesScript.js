...
var cookie_preferences={essentials:true,usage:false,marketing:false};/**
     ...
     */function optUserIn(){hideMessage();updateSeenCookie();fireGTM()}function fireGTM(){(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({"gtm.start":new Date().getTime(),event:"gtm.js"});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!="dataLayer"?"&l="+l:"";j.async=true;j.src="https://www.googletagmanager.com/gtm.js?id="+i+dl;f.parentNode.insertBefore(j,f)})(window,document,"script","dataLayer","GTM-5NQGRQN")}function updateSeenCookie(){// 1 year = 3.154e+7
// 1 month = 2.628e+6
...
if(!docCookies.hasItem("cookie_preferences_set")){var cookie_preferences_accepted={essentials:true,usage:true,marketing:true};docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences_accepted),oneyear,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
// Set the 'cookies_timer_reset' to prevent showing the banner again next time the user visits
docCookies.setItem("cookies_timer_reset",JSON.stringify(true),oneyear,"/",".crowncommercial.gov.uk");docCookies.setItem("seen_cookie_message",true,oneyear,"/",".crowncommercial.gov.uk")}// this is a case when the user selected cookies on the cookie settings page â€“ the cookie_preferences array 
// will be updated with this user's choices
else{// console.log('cookie_preferences', cookie_preferences);
var cookie_timer=cookie_preferences["marketing"]===false&&cookie_preferences["usage"]===false?twodays:oneyear;docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences),cookie_timer,"/",".crowncommercial.gov.uk");// Set the 'cookies_timer_reset' to prevent showing the banner again next time the user visits
docCookies.setItem("cookies_timer_reset",JSON.stringify(true),cookie_timer,"/",".crowncommercial.gov.uk");docCookies.setItem("seen_cookie_message",true,cookie_timer,"/",".crowncommercial.gov.uk")}}function deleteDisabledCookies(cookie_type){// Loop through each cookie group
...
if(cookieGroup.cookie_type===cookie_type&&cookieGroup.cookies!==null){// console.log({cookieGroup});
...
})}})}function UpdateCookiePreferences(){// update the cookie references based on user selection
...
deleteDisabledCookies(datarecord.cookie_type)}}});var cookie_timer=cookie_preferences["usage"]===false&&cookie_preferences["marketing"]===false?twodays:oneyear;docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences),cookie_timer,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
...
}var SettingsUpdatedArea=document.getElementsByClassName("js-live-area");SettingsUpdatedArea[0].innerHTML="<p>Your cookie settings were saved.</p>"}function generateCookieSettingsPageContent(appendTo){var CookieSettingsPageContent=document.createDocumentFragment();var cookie_preferences=JSON.parse(docCookies.getItem("cookie_preferences"));// console.log({cookie_preferences});
...
appendTo.innerHTML="";appendTo.appendChild(CookieSettingsPageContent)}/**
     ...
     */function createCookieMessage(){// create the cookie message container
var cookieMessageContainer=document.createElement("div");cookieMessageContainer.classList.add("cookie-message");cookieMessageContainer.setAttribute("id","cookie-consent-container");cookieMessageContainer.setAttribute("aria-label","Cookie Policy");cookieMessageContainer.setAttribute("aria-live","polite");// create the inner contents of the cookie message
var cookieMessageInner=document.createElement("div");cookieMessageInner.classList.add("cookie-message__inner","govuk-width-container");cookieMessageInner.classList.add("site-container");cookieMessageInner.innerHTML="<h2 class=\"govuk-heading-m\">Cookies on crowncommercial.gov.uk</h2><div class=\"cookie-message__intro\"><p>We use cookies to collect information about how you use crowncommercial.gov.uk.<br><br> We use this information to make the website work as well as possible and improve government services.</p></div>";var optInButton=document.createElement("button");optInButton.classList.add("govuk-!-font-size-18","govuk-!-font-weight-bold","govuk-button","gtm--accept-cookies-in-banner");optInButton.innerHTML="Accept all cookies";optInButton.addEventListener("click",optUserIn);var settingsButton=document.createElement("a");settingsButton.classList.add("govuk-!-font-size-18","govuk-!-font-weight-bold","govuk-button");settingsButton.setAttribute("href","/cookie-settings");settingsButton.innerHTML="Set cookie preferences";// optOutButton.classList.add('button');
...
var cookieMessageButtons=document.createElement("div");cookieMessageButtons.classList.add("cookie-message__actions");cookieMessageButtons.appendChild(optInButton);cookieMessageButtons.appendChild(settingsButton);cookieMessageInner.appendChild(cookieMessageButtons);// append the inner contents to the cookie message container
...
var toContent=document.getElementById("skiplink-container");document.body.insertBefore(cookieMessageContainer,toContent);cookieMessageContainer.style.display="block"}/**
     ...
     */if(!docCookies.hasItem("seen_cookie_message")&&window.location.href.indexOf("cookie-settings")===-1){createCookieMessage()}// Only set the default cookies if they haven't been set
if(!docCookies.hasItem("cookie_preferences")){docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences),twodays,"/",".crowncommercial.gov.uk");// docCookies.setItem('cookies_timer_reset', JSON.stringify(cookie_preferences), oneyear, '/', '.crowncommercial.gov.uk');
// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
     ...
     */if(docCookies.hasItem("seen_cookie_message")&&!docCookies.hasItem("cookies_timer_reset")){// If not on the cookie settings page, show the banner;
if(window.location.href.indexOf("cookie-settings")===-1){createCookieMessage()}}/**
