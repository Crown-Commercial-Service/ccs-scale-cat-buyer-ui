            ...
            if(preventDefaultState.length > 0 || decimalnumber.length > 0 || nonnumerical.length > 0 || rolevalidation.length>0 || staffval.length>0 || vettval.length> 0 || rolebox_validation.length>0)
{
                ...
                errormsg=errormsg+'<li><a href="#'+staffval[0].id+'">Sum of all [Weighting for number of staff] values for all Role Families in all groups = 100%</a></li>';
  
                if(vettval.length>0)

                errormsg=errormsg+'<li><a href="#'+vettval[0].id+'">Sum of all [Weighting for related vetting requirement] values for all Role Families in all groups = 100%</a></li>';
        ...
        if (!inputtedtext.length > 0) {
            ...
            $('#ca_vetting_error_summary').removeClass('hide-block');
            $('.govuk-error-summary__title').text('There is a problem');
            $("#ca_summary_list").html('You must enter atleast 1 role');
            $('html, body').animate({ scrollTop: 0 }, 'fast');
        }
