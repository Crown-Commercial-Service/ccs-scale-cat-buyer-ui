document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById("ccs-eoi-suppliers-form") !== null) {
        ...
        document.getElementById("eoi_page-1-next-link").addEventListener('click', (e) => {
            e.preventDefault();
            supplier_page1.classList.add("ccs-dynaform-hidden");
            supplier_page2.classList.remove("ccs-dynaform-hidden")
        });

        document.getElementById("eoi_page-2-previous-link").addEventListener('click', (e) => {
            e.preventDefault();
            supplier_page1.classList.remove("ccs-dynaform-hidden");
            supplier_page2.classList.add("ccs-dynaform-hidden")
        });

        document.getElementById("eoi_page-2-next-link").addEventListener('click', (e) => {
            e.preventDefault();
            supplier_page2.classList.add("ccs-dynaform-hidden")
            supplier_page3.classList.remove("ccs-dynaform-hidden");
        });

        document.getElementById("eoi_page-3-previous-link").addEventListener('click', (e) => {
            e.preventDefault();
            supplier_page3.classList.add("ccs-dynaform-hidden")
            supplier_page2.classList.remove("ccs-dynaform-hidden");
        });
