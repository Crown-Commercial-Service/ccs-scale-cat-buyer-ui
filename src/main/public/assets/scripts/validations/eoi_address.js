...
const ccsZFindAddress = (event) => {
  ...
  if (fieldCheck !== true) {
    ...
    ccsZPresentErrorSummary([["eoi_proj_address-postcode", "Enter a valid postcode"]]);
  } else {
    // if we have a valid postcode, remove any error mssages
    ccsZremoveErrorMessage(document.getElementById("eoi_proj_address-postcode"));
    ...
    let myPostCode = document.getElementById("eoi_proj_address-postcode").value.trim();
    ...
    fetch(`https://api.getAddress.io/find/${myPostCode}?api-key=`)
      ...
      .then(data => {
        ...
        let mySelector = document.getElementById("eoi_proj_address-address"),
        ...
        document.getElementById("eoi_proj_address-postcode").classList.add("ccs-dynaform-hidden");
        document.getElementById("eoi_find_address_btn").classList.add("ccs-dynaform-hidden");
...
const ccsZFoundAddress = (event) => {
  let saveButton = document.getElementById("eoi_save_address_btn");
...
const ccsZResetAddress = (event) => {
  ...
  document.getElementById("eoi_proj_address-postcode").classList.remove("ccs-dynaform-hidden");
  document.getElementById("eoi_find_address_btn").classList.remove("ccs-dynaform-hidden");
  document.getElementById("eoi_proj_address-address").parentNode.classList.add("ccs-dynaform-hidden");
  document.getElementById("eoi_proj_address-postcode").value = "";
  document.getElementById("eoi_proj_address-postcode").focus();
