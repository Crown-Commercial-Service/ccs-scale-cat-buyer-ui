...
$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    $('#sent-message tbody tr').css('opacity', '0.0').hide().slice(startItem, endItem).
     ...
     $(document).ready(function () {
      $('#received-message').after('<div class="ccs-pagination" id="historical-nav"><p class="govuk-visually-hidden" aria-labelledby="pagination-label">Pagination historical-navigation</p></div>');
          var rowsShown = 10;
      var rowsTotal = $('#received-message tbody tr').length;
          var numPages = rowsTotal / rowsShown;
          var pageNum = 0;
      $('#historical-nav').append('<a id="received-message-previous" rel="0" class="ccs-pagination__link" href="#">Previous<span class="govuk-visually-hidden"> set of pages</span></a>');
          for (i = 0; i < numPages; i++) {
            pageNum = i + 1;
            $('#historical-nav').append('<a class="ccs-pagination__link" id="pageId_'+i+'" href="#" rel="' + i + '">' + pageNum + '</a> ');
          }
      $('#historical-nav').append('<a id="received-message-next" rel="1" class="ccs-pagination__link" href="#">Next<span class="govuk-visually-hidden"> set of pages</span></a>');
