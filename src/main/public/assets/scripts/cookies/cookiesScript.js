...
(function() {
    ...
    function deleteDisabledCookies(cookie_type) { // Loop through each cookie group
    ...
    if (!docCookies.hasItem("cookie_preferences")) {
          ...
          const sKey = 'cookie_preferences';
          const sValue = JSON.stringify(cookie_preferences);
          const sExpires = twodays;
          const sDomain = "/";
          const sPath = ".crowncommercial.gov.uk"
    ...
    for (var i = 0; i < cookieConsentAreas.length; i++) { // generateStatusMessage(cookieConsentAreas[i]);
