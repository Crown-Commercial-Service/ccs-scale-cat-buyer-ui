...
export class Oauth_Instance {
  ...
  static tokenCheck_AuthService_BaseURL =
    process.env.AUTH_SERVER_BASE_URL + config.get('authenticationService.token_validation_endpoint');
  static userProfile_AuthService_BaseURL =
    process.env.AUTH_SERVER_BASE_URL + config.get('authenticationService.user_profile_endpoint');
  ...
  static TokenWithApiKeyInstance = (api_key: string, emailid: string): axios.AxiosInstance => {
    const BaseURL = `${Oauth_Instance.userProfile_AuthService_BaseURL}?user-id=${encodeURIComponent(emailid)}`;
    return axios.default.create({
      baseURL: BaseURL,
      headers: {
        'Content-Type': 'application/json',
