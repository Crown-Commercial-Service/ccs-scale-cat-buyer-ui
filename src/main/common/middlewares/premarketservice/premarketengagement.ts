...
export class PreMarketEngagementMiddleware {
  ...
  req.session.stage2_value = stage2_value;
    ...
    if(agreement_id == 'RM6187') { //MCF3
      const {data: getEventsData} = await TenderApi.Instance(req.cookies.SESSION_ID).get(`tenders/projects/${projectId}/events`);
      let { data: journeySteps } = await TenderApi.Instance(req.cookies.SESSION_ID).get(`journeys/${eventId}/steps`);
    ...
    if(agreement_id == 'RM1043.8' && req.session.dosStage =='Stage 2') { //DOS
        ...
        try {
          ...
          if (getEventType === 'TBD') {
            ...
            if(agreement_id == 'RM1043.8') {
              const {data: getEventsData} = await TenderApi.Instance(req.cookies.SESSION_ID).get(`tenders/projects/${projectId}/events`);
              const isStage_1 = getEventsData.some(item => item.eventType == 'FC' && item.dashboardStatus == 'EVALUATED');
              if(isStage_1) {
                // Stage-2
                if (eventType === 'FC') {

                  const DOS6_FC = eventTypes.find(ev => ev.type === 'FC');
                  let inner = DOS6_FC.templateGroups;
                  const outResult = inner.find((el) => el.inheritsFrom);
                   let templeateID = outResult.templateGroupId;
            ...
            req.session.procurements[currentProcNum].started = true;
            next();
          } else if (getEventType === 'FCA' || getEventType === 'PA' || getEventType === 'DAA' || getEventType === 'FC' || getEventType === 'DA') {
            //For DOS6 Stage - 2
            if(agreement_id == 'RM1043.8') {
              const {data: getEventsData} = await TenderApi.Instance(req.cookies.SESSION_ID).get(`tenders/projects/${projectId}/events`);
              const isStage_1 = getEventsData.some(item => item.eventType == 'FC' && item.dashboardStatus == 'EVALUATED');
          ...
          if(agreement_id == 'RM6187') { //MCF3
            if(getEventType === 'PA') {
              ...
              let PushAssessmentID = data.assessmentId;
              //Old & new joureny logic
              const {data: currentEventTrack}  = await TenderApi.Instance(SESSION_ID).get(eventTypeURL);
              const overWritePaJoury = currentEventTrack.find(item => item.eventType == 'PA' && (item.dashboardStatus == 'CLOSED' || item.dashboardStatus == 'COMPLETE'));
              let journyDataStates;
              if(overWritePaJoury)  {
                let PAEventId = overWritePaJoury.id;
                const { data: journeyPlugFromPA } = await TenderApi.Instance(SESSION_ID).get(`journeys/${PAEventId}/steps`);
          ...
          if(agreement_id == 'RM1043.8') { //DOS
            if(getEventType === 'FC') {
              ...
              //Old & new joureny logic
              const {data: currentEventTrack}  = await TenderApi.Instance(SESSION_ID).get(eventTypeURL);
              const overWriteFcJoury = currentEventTrack.find(item => item.eventType == 'FC' && (item.dashboardStatus == 'CLOSED' || item.dashboardStatus == 'COMPLETE'));
              let journyDataStates;
              if(overWriteFcJoury)  {
                let FCEventId = overWriteFcJoury.id;
                const { data: journeyPlugFromFC } = await TenderApi.Instance(SESSION_ID).get(`journeys/${FCEventId}/steps`);
