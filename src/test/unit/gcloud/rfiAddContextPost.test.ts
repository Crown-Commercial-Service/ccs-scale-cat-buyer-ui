...
import { postData } from 'test/data/gcloud/rfiGcloudQuestionPostData.json';
//const contextnonOCDS = require('test/utils/gcloud/qsData').context_datas_gcloud
const contextnonOCDS = require('test/utils/gcloud/qsData').context_nonOCDS_gcloud
  ...
  for(let i=0;i < postData.length;i++){
    it(`Post : expect Controller at endpoint ${postData[i]?.OCDS?.id} to return success status`, async () => {
      //nock(envs.TENDERS_SERVICE_API_URL).put(`/journeys/${eventId}/steps/20`).reply(200, true);
        ...
        .post(`/rfi/questionnaire?agreement_id=RM1557.13&proc_id=${procId}&event_id=${eventId}&id=Criterion 1&group_id=${postData[i]?.OCDS?.id}`)
