...
import { AuthCredentials } from 'main/services/types/publicProcurementGateway/oAuth/api';
...
describe('OAuth API helpers', () => {
  ...
  describe('refreshToken', () => {
    it('calls the post refresh token endpoint with the correct url and headers', async () => {
      ...
      const refreshTokenResult = await oAuthAPI.refreshToken(authCredentials);

      expect(refreshTokenResult).to.eql({
        status: FetchResultStatus.OK,
        data: { access_token: 'myAccessToken', session_state: 'mySessionState', refresh_token: 'myRefreshToken' }
      });
    });
  ...
  describe('validateToken', () => {
    it('calls the post refresh token endpoint with the correct url and headers', async () => {
      ...
      const validateTokenResult = await oAuthAPI.validateToken(accessToken);

      expect(validateTokenResult).to.eql({
        status: FetchResultStatus.OK,
        data: true
      });
    });
