...
import { matchHeaders, matchJSON, matchQueryParams } from 'specs/support/mswMatchers';
    ...
    describe('when the response is 200', () => {
      ...
      describe('and there are cache params', () => {
        ...
        it('calls the redis cache and the endpoint when there is no data', async () => {
          ...
          assertRedisCalls(redisSpys, 'my_cache_key', { ok: 'GET /test' }, 394);
        });

        it('calls only the redis cache when there is data', async () => {
          const redisSpys = creatRedisMockSpys(vi, { ok: 'GET /test' });
          ...
          assertRedisCallsWithCache(redisSpys, 'my_cache_key');
        });
      });
      ...
      describe('and we consider logger options', () => {
        ...
        it('calls the logger when it has no logger options', async () => {
          ...
          assertPerformanceLoggerCalls(performanceLoggerSpys, undefined);
        });
      });
    });
